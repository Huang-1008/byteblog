...
- 🔴 **不要在用户确认前写任何业务规则代码。**
- 🔴 **MySQL MCP 只读也要用！用它验证每一步：表在不在、列对不对、数据有没有乱码。**
- 🔴 **数据库写操作用 pymysql 脚本，但验证必须用 MCP 重复确认。**
- 🔴 **Playwright CLI 是首选截图工具，MCP 是备选。** `npx playwright screenshot --browser=chromium <url> <file>`
