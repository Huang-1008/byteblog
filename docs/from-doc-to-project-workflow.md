...
| 配置项 | 用途 | 示例 | 影响范围 |
|------|------|------|------|
| **AI API Key** | DeepSeek/OpenAI 调用 | `sk-xxxxx` | AI 功能 |
| **MySQL 密码** | 数据库连接 | `12345678` | 全部功能 |
| **GitHub 用户名** | 创建仓库 + 推送 | `Huang123` | 版本控制 |
| **GitHub Token**（可选） | MCP 自动建仓库 | `ghp_xxxx` | 版本控制 |
...
```
在开始写代码前，我需要以下信息：
1. 你的 DeepSeek/OpenAI API Key（有就用，没有就跳过 AI 功能）
2. 你的 MySQL root 密码
3. 你的 GitHub 用户名（建仓库用）
4. 你的学校、班级、姓名、学号（论文封面用）

> ⚠️ 密码和 Key 只在本地 .env 使用，不会提交到 GitHub。
```
...
3. 写入论文封面脚本（学校/班级/姓名/学号）
4. 写一个 .env.example 模板（不含真实密码，供 Git 提交）
```

> ⚠️ 不提前要密码的后果：自动模式的安全检测会反复拦截所有 Bash 命令，导致"读日志→修Bug→重启"循环无法进行，整个验证阶段瘫痪。
